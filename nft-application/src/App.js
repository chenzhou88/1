...
//  合约地址
const MYNFT_ADDRESS = "0x85A35128A804A35194c7f1eF87eb83296DF345ca";
const MARKET_ADDRESS = "0x1fA8Fd279a6d318c9045D1B33fE5D86dc1BfFa7A";
...
function App() {
  ...
  return (
    <div className="App">
      <h1 className="app-header">NFT 交易平台</h1>
      {!account? (
        <button className="connect-wallet-btn" onClick={connectWallet}>Connect Wallet</button>
      ) : (
        <div>
          <p className="connected-account">Connected as {account}</p>
          <h2 className="section-header">铸造一个 NFT</h2>
          <input
            type="text"
          ...
          />
          <button className="mint-nft-btn" onClick={mintNFT} disabled={loading}>
            {loading? 'Minting...' : 'Mint NFT'}
          </button>

          <h2 className="section-header">出售 NFT</h2>
          <input
            type="number"
          ...
          <input
            ...
            className="input-field"
          />
          <button className="list-nft-btn" onClick={listNFTForSale} disabled={loading}>
            {loading? 'Listing...' : 'List NFT'}
          </button>

          <h2 className="section-header">你的 NFT</h2>
          {userNFTs.length === 0? (
            <p className="no-nft-text">你还没有自己的 NFT</p>
          ) : (
          ...
          <h2 className="section-header">正在出售的 NFT</h2>
          {nftsForSale.length === 0? (
            <p className="no-sale-nft-text">无 NFT 出售中</p>
          ) : (
            <ul className="nft-sale-list">
              {nftsForSale.map(({ tokenId, price }) => (
                <li key={tokenId.toString()} className="nft-sale-item">
                  Token ID: {tokenId.toString()}, Price: {price.toString()} ETH
                  <button className="buy-nft-btn" onClick={() => buyNFT(tokenId, price)} disabled={loading}>
